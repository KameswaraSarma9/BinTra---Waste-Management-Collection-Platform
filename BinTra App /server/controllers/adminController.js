...
exports.assignDriverToTruck = async (req, res) => {
  try {
    ...
    // Don't let an in-progress pickup get orphaned by a reassignment.
    const activeStates = ["ASSIGNED", "COLLECTING", "RETURNING_TO_YARD"];
    if (activeStates.includes(truck.status) && String(truck.driver) !== String(driver._id)) {
      return res.status(400).json({
        message: "This truck has an active pickup in progress and cannot be reassigned right now",
      });
    }

    // If this truck previously had a different driver, unassign that driver first
    // so we never leave a driver pointing at a truck that no longer has them.
    if (truck.driver && String(truck.driver) !== String(driver._id)) {
      await User.findByIdAndUpdate(truck.driver, { assignedTruck: null });
    }

    // If this driver was previously assigned to a different truck, free that
    // truck's driver slot too - a driver should never be on two trucks at once.
    if (driver.assignedTruck && String(driver.assignedTruck) !== String(truck._id)) {
    ...
    // Assigning a driver does NOT make the truck available - it starts OFFLINE
    // until the driver explicitly goes online for their shift.
    ...
    res.json({ message: "Driver assigned to truck. Truck is OFFLINE until the driver goes online.", truck });
  } catch (err) {
