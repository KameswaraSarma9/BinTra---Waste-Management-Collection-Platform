...
export default function DriverDashboard() {
  ...
  // Always load the driver's truck from the backend - never from
  // user.assignedTruck cached in localStorage, which can go stale the moment
  // an admin reassigns trucks/drivers.
  const loadTruck = () => {
  ...
  const goOnline = () => {
    if (!navigator.geolocation) {
      notify("Your browser doesn't support location, cannot go online", true);
      return;
    }
    setGoingOnline(true);
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        try {
          const { data } = await api.patch("/trucks/go-online", {
            lat: pos.coords.latitude,
            lng: pos.coords.longitude,
          });
          setTruck(data);
          notify("You're online. Users can now find your truck nearby.");
        } catch (err) {
          notify(err.response?.data?.message || "Failed to go online", true);
        } finally {
          setGoingOnline(false);
        }
      },
      () => {
        notify("Location permission is required to go online", true);
        setGoingOnline(false);
      }
    );
  };
  ...
  const goOffline = async () => {
    try {
      const { data } = await api.patch("/trucks/go-offline");
      setTruck(data);
      notify("You're offline.");
    } catch (err) {
      notify(err.response?.data?.message || "Failed to go offline", true);
    }
  };

  const arriveAtYard = async () => {
    if (!truck?._id || !selectedYard) {
  ...
  return (
    <div className="container">
      ...
      <div className="card">
        <h3>My truck</h3>
        {truckLoading && <p>Loading your truck...</p>}
        {!truckLoading && !truck && (
          <p>No truck is assigned to you yet. Ask an admin to assign one.</p>
        )}
        {!truckLoading && truck && (
          <>
            <p>Vehicle: <strong>{truck.vehicleNumber}</strong></p>
            <p>Status: <span className="badge">{truck.status}</span> ({truck.isAvailable ? "Available" : "Not available"})</p>
            <p>Load: {truck.currentLoad}kg / {truck.capacity}kg</p>
            <p>Location: {truck.currentLocation?.lat?.toFixed?.(4)}, {truck.currentLocation?.lng?.toFixed?.(4)}</p>
            {truck.status === "OFFLINE" ? (
              <button disabled={goingOnline} onClick={goOnline}>
                {goingOnline ? "Getting your location..." : "Go Online"}
              </button>
            ) : (
